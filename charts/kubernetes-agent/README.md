...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| 3         | [main](https://github.com/OctopusDeploy/helm-charts/tree/main/charts/kubernetes-agent)                                               | This file                                                                                                               | [here](./values.yaml)                                                                                                     |
| 2         | [release/kubernetes-agent/v1](https://github.com/OctopusDeploy/helm-charts/tree/release/kubernetes-agent/v2/charts/kubernetes-agent) | [here](https://github.com/OctopusDeploy/helm-charts/blob/release/kubernetes-agent/v2/charts/kubernetes-agent/README.md) | [here](https://github.com/OctopusDeploy/helm-charts/blob/release/kubernetes-agent/v2/charts/kubernetes-agent/values.yaml) |
...
Major versions of the Helm chart introduce breaking changes to `values.yaml`; some elements were renamed, while others were moved.

As such, upgrading between major versions of the Helm chart requires user intervention.
...
The in-cluster NFS server has been deprecated and will be removed in a future major release. See https://github.com/OctopusDeploy/helm-charts/issues/554

## Sub-charts

The Kubernetes monitor is optionally installed alongside the Kubernetes agent, [read more here](https://github.com/OctopusDeploy/kubernetes-monitor).
...
| agent.containers.tentacle.env | list | `[]` | Additional env to apply to the tentacle container - does not override any other configuration |
| agent.containers.tentacle.spec | object | `{}` | Additional container spec to apply to the tentacle container - does not override any other configuration |
| agent.containers.watchdog.env | list | `[]` | Additional env to apply to the watchdog container - does not override any other configuration |
...
| agent.readinessProbe | object | `{"enabled":false,"failureThreshold":3,"initialDelaySeconds":30,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | Readiness probe for polling Tentacles. Polling Tentacles expose no port to check directly, so readiness is determined by an exec check that the Tentacle process is running. |
| agent.readinessProbe.enabled | bool | `false` | Set to true to enable the readiness probe. |
| agent.readinessProbe.failureThreshold | int | `3` | Number of consecutive probe failures before the pod is marked not ready. |
| agent.readinessProbe.initialDelaySeconds | int | `30` | Seconds after the container starts before the first probe runs. |
| agent.readinessProbe.periodSeconds | int | `10` | How often (in seconds) to perform the probe. |
| agent.readinessProbe.successThreshold | int | `1` | Number of consecutive successes required to mark the pod ready again after failing. |
| agent.readinessProbe.timeoutSeconds | int | `5` | Seconds after which the probe times out. |
...
| agent.spec | object | `{}` | Additional pod spec to apply to the agent pod - does not override any other agent configuration |
...
| persistence.storageClassName | string | `""` | When set to "-", results in storageClassName: "" being set in the PVC. |
| persistence.volumeName | string | `""` | When NFS is disabled, will set the provided volumeName in the PVC. |
