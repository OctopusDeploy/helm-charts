...
| agent.readinessProbe | object | `{"failureThreshold":3,"initialDelaySeconds":30,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | Readiness probe for polling Tentacles. Polling Tentacles expose no port to check directly, so readiness is determined by an exec check that the Tentacle process is running. |
