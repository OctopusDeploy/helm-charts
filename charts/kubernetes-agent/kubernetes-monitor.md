...
| monitor.disableGrpcCompression | bool | `false` | Send gRPC messages to Octopus Server uncompressed. When true, sets the DISABLE_GRPC_COMPRESSION environment variable on the monitor pod. Only use this for troubleshooting. |
...
| monitor.healthCheck.giveUpAfter | string | `""` | How long Octopus Server may go unanswered before the monitor exits and the pod is restarted, as a duration such as "5m" or "1h". Sets the HEALTH_CHECK_GIVE_UP_AFTER environment variable on the monitor pod. Leave empty to use the monitor's default of 5 minutes. |
| monitor.healthCheck.interval | string | `""` | How often the monitor sends a gRPC health check to Octopus Server, as a duration such as "30s" or "1m". Sets the HEALTH_CHECK_INTERVAL environment variable on the monitor pod. Set to "0" to disable the health check, which should only be for troubleshooting (error-recovery is limited when health checks are disabled). Leave empty to use the monitor's default of 30 seconds. |
...
| monitor.maxGrpcReceiveMessageSizeBytes | string | `""` | Largest gRPC message the monitor will accept from Octopus Server, in bytes. Sets the MAX_GRPC_RECEIVE_MESSAGE_SIZE_BYTES environment variable on the monitor pod. Leave empty to use the monitor's default of 20MiB. |
| monitor.maxGrpcSendMessageSizeBytes | string | `""` | Largest gRPC message the monitor will send to Octopus Server, in bytes. Sets the MAX_GRPC_SEND_MESSAGE_SIZE_BYTES environment variable on the monitor pod. Leave empty to use the monitor's default of 50MiB. |
